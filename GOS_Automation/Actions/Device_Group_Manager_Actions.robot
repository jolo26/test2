...
Search Device using search field
    ...
    Wait And Click Element    ${label_search_field.replace("test_id","select-searchField")}
    Wait And Click Element    ${search_field_drop_down_options.replace("value", "${label}")}
...
Click Searched Device Group
    [Arguments]    ${device_group}
            Wait and Click Element    ${device_group_first_result_table.replace("value","${device_group}")}

Verify Device Group Manager Details Page Component
    Wait Until Element Is Visible     ${details_title.replace('title', 'Device Group Manager')}
    Wait Until Element Is Visible     ${details_label_with_label.replace('field_label', 'Product Name')}
    Wait Until Element Is Visible     ${details_label_with_label.replace('field_label', 'Device Mapping')}
    Wait Until Element Is Visible     ${details_label.replace('field_label', 'Created')}
    Wait Until Element Is Visible     ${details_label.replace('field_label', 'Device Group Name')}
